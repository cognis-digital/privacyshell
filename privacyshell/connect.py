"""Native cognis-connect emit for privacyshell — forward findings to any platform.

Maps privacyshell's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    privacyshell ... --format json | privacyshell-emit --to stix
    privacyshell-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop('source', None)
        out.pop('tags', None)
        out['title'] = rec.get('title', 'Unknown')
