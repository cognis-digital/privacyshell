...
**privacyshell** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `privacyshell` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
privacyshell ... --format json | cognis-connect emit --to sigma   # Sigma rules
privacyshell ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
privacyshell ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
privacyshell ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `privacyshell` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="privacyshell") for rec in records]   # records = your JSON output
